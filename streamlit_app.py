...
def step_region():
    ...
    for i, (name, sub) in enumerate(region_list):
        col = cols[i % 2]
        with col:
            selected = st.session_state.region == name
            border = "border: 1.5px solid #1D9E75; background: #E1F5EE;" if selected else "border: 0.5px solid #e0e0e0; background: white;"
            st.markdown(f"""
                <div style="border-radius:12px;padding:14px 16px;margin-bottom:10px;cursor:pointer;{border}">
                    <div style="font-size:1.1rem;margin-bottom:5px;">{name.split()[0]}</div>
                    <div style="font-weight:500;font-size:0.9rem;margin-bottom:2px;">{" ".join(name.split()[1:])}</div>
                    <div style="font-size:0.78rem;color:#888;">{sub}</div>
                </div>""", unsafe_allow_html=True)
            if st.button(f"Select", key=f"region_{i}"):
                st.session_state.region = name
                st.rerun()

    st.markdown("<br>", unsafe_allow_html=True)
    if st.session_state.region:
        st.success(f"Selected: **{st.session_state.region}**")
